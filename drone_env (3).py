...
class AirSimDroneEnv(gym.Env):
    def __init__(self, ip_address, image_shape):
        ...
        self.observation_space = gym.spaces.Box(low=0, 
        	high=255, shape=self.image_shape, dtype=np.uint8)
        self.action_space = gym.spaces.Box(np.array([-1,-1,-1]).
        	astype(np.float32), np.array([-1,+1,+1]).astype(np.float32))
    ...
    def setup_flight(self):
        ...
        #self.target_pos = [100,random.randint(-50,50),-5]
        
        self.target_pos = [20,30,0]
        pose = airsim.Pose(airsim.Vector3r(0, 0, 0))
        self.drone.simSetVehiclePose(pose=pose, ignore_collision=True)
        self.target_dist_prev = np.linalg.norm(np.array([0,0,0]) - self.target_pos)
        self.target_dist_prev = abs(self.target_dist_prev)
